...
<nav class="navbar navbar-default navbar-fixed-top large" id="main-nav"> 
  ...
  <ul class='dropdown-menu' aria-labelledby='dLabel'>
    <a><li style='padding-left:5px;'>Edit your profile</li></a>
    <a><li style='padding-left:5px;'>View your profile</li></a>
    ...
    <div class="panel">
      ...
      <?php 
						if(isset($_SESSION['email']))
						{
							...
							$check_user="select java_enroll from users where u_email='$email'";
							$run_check=mysqli_query($conn,$check_user);
							$row=mysqli_fetch_array($run_check);
							if($row['java_enroll']==NULL)
							{
    ...
    <?php 
					if(isset($_POST['enroll']))
					{
						if(isset($_SESSION['email']))
						{
							...
							if($row['java_enroll']==NULL)
							{
								$sql = "UPDATE users SET java_enroll=1 WHERE u_email='$email'";	
								mysqli_query($conn,$sql);
								echo "<script>window.open('java-content.php','_self')</script>";
							}
							else
							{
								 }
